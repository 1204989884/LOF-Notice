    ...
    async with _cache_lock:
        ...
        try:
            ...
            if result.get("data"):
                ...
                for t in new_trades:
                    print(f"[买入] {t['code']} {t['name']} | 净值={t['buy_nav']} | 溢价={t['premium_rt_at_buy']}% | 投入¥{t['buy_amount']}")
                ...
                if holding_codes:
                    ...
                    # 兜底：不在当前数据中的持仓从腾讯财经获取价格
                    for code in holding_codes:
                        if code not in prices:
                            fetched = await _fetch_price_from_tencent(code)
                            if fetched > 0:
                                prices[code] = fetched
                    if prices:
                ...
                # 自动卖出T+2到期的持仓
                holdings_summary = get_holdings_summary()
                sellable = [h for h in holdings_summary.get("holdings", []) if h.get("can_sell") and h.get("current_price", 0) > 0]
                for h in sellable:
                    sell_result = record_sell(
                        code=h["code"],
                    ...
                    if sell_result:
                        ...
                        print(f"[卖出] {h['code']} {h['name']} | 卖价={h['current_price']:.4f} | 盈亏¥{pnl:+.2f} | 持{h['days_held']}天")
                if sellable:
                    print(f"[卖出] T+2到期自动平仓 {len(sellable)} 只")
...
async def _fetch_price_from_tencent(code: str) -> float:
    """从腾讯财经获取单只LOF实时价格"""
    try:
        import httpx
        import re
...
async def sell_position(code: str, price: float = Query(0, description="卖出价格，0表示用当前价")):
    ...
    # 缓存中找不到，从腾讯财经获取实时价格
    if price <= 0:
        price = await _fetch_price_from_tencent(code)
